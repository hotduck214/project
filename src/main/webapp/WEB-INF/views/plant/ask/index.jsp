                ...
                <div class="bbs">
                    <table class="list">
                        ...
                        <c:forEach var="vo" items="${data.list }" varStatus="status">
                            <tr>
                                <td>${data.totalCount-status.index-(askVO.page-1)*askVO.pageRow }<!-- 총개수 - 인덱스-(현재페이지번호-1)*페이지당개수 --></td>
                                
                                <td class="txt_l">
                                    <a href="view.do?ask_no=${vo.ask_no }">${vo.ask_title }</a>
                                </td>
                                ...
                                <td class="date"><fmt:formatDate value="${vo.ask_regdate }" pattern="yyyy-MM-dd HH:mm:ss"/></td>
                            </tr>
                        </c:forEach>
                        </tbody>
                    </table>
     				<div class="btnSet"  style="text-align:right;">                   		
     					<p><span><strong>총${data.totalCount }개</strong>  | ${askVO.page }/${ data.totalPage} 페이지</span></p>     
                    ...
                    <div class="pagenate clear">
                      ...
                      <c:if test="${data.prev == true }">
                        ...
                        <c:forEach var="p" begin="${data.startPage }" end="${data.endPage }">
                            <li><a href='index.do?page=${p }&stype=${param.stype}&sword=${param.sword}' <c:if test="${askVO.page == p }">class='current'</c:if>>${p }</a></li>
                        </c:forEach>
